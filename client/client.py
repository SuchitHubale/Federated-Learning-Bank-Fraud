...
class FederatedClient:
    def __init__(self, client_id, server_url, data_path, model_save_path, epochs=10):
    ...
    def train(self):
        ...
        trained_model = train_model(
            model, X_train, y_train, X_test, y_test, epochs=self.epochs
        )
    ...
    def get_global_model(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"Client {self.client_id}: Failed to get global model, status: {response.status_code}")
                return False
                
        except Exception as e:
            print(f"Client {self.client_id}: Error getting global model - {str(e)}")
            return False
